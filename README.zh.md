...
**构建还在跑,代理不该干等着。** 这就是 Claude Code 的后台任务体验,如今搬到了 Pi 上:扔出一条长命令,它不会卡住整个会话,而是悄悄溜到后台,代理则继续埋头干活。30 秒自动转后台、Ctrl+Shift+B 一键秒转、输出全程捕获、卡顿自动检测,外加一个功能齐全的作业管理器 —— 全都打包进一个扩展里。
...
**会话卡死,到此为止。** 开发服务器、测试套件、构建 —— 任何跑过 30 秒还没完事的命令,都会被悄悄挪到后台。代理收到提醒,转头就去干下一件事,而不是盯着转圈圈的进度条发呆。想让它早点滚到后台?随时手动一推就行。
...
内置的 bash 工具,但多了点求生本能。命令照常运行 —— 可一旦某条冲破 30 秒,它就悄悄滑进后台。没有决策提示,也不强制开新轮:工具结果本身(`Command running in background with ID: …`)就告诉代理输出去了哪里。

| 参数 | 说明 |
|------|------|
| `command` | 要运行的 shell 命令 |
| `timeout` | 转后台前最长等待(秒,默认:120,硬上限 30) |
...
- 前台只留给快命令。预期较长的任务请用 `run_in_background`(一次完成通知)或 `monitor` 工具(逐事件流式)。
- 不变:2 秒快速完成窗口、`sleep` 黑名单(不可转后台的命令仍在等待上限处被 kill)、后台任务生命周期。
...
> 基于 [pi-patty-bg-tasks](https://github.com/patty-io/pi-patty-bg-tasks)（原作者 Patty）个人维护的分支。
